...
9. 默认语言为英文：仓库主 README 使用英文（`README.md`），中文放在 `README_ch.md`，并在顶部双向跳转。
10. 双语一致性：中英文 README 结构应基本一致，关键信息（数量、路径、规则）必须同步。
11. 视觉吸引力：允许使用适量 emoji 强化可读性（标题、分组、亮点），但避免堆砌和干扰信息检索。
12. 酷炫但务实：优先使用“短标语 + 亮点列表 + 快速概览表 + 检查清单”的结构，不牺牲准确性。

### README 风格建议（中英文通用）

1. 开头使用一句 slogan（可配 emoji）快速建立定位。
2. 设置 `At A Glance / 快速概览` 表格，优先展示总数、分类、入口文档。
3. 关键板块标题可使用语义 emoji（如 `✨`、`📌`、`🗂️`、`🔗`、`✅`）。
4. 每段尽量短句，强调可扫描性；列表优先，减少长段叙述。
5. “吸引人”不等于“花哨”：数据、路径、命名规则仍是第一优先级。
