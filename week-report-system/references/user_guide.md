...
├── 2024/
│   ├── week01/
│   │   ├── 20240101-a1b2c3d4.txt    # 1月1日开始的会话（简略记录）
│   │   ├── 20240103-e5f6g7h8.txt    # 1月3日开始的会话（简略记录）
│   │   └── report-20240107-183000.md # 生成的周报（带时间戳，避免多端冲突）
│   ├── week02/
│   └── ...
└── 2025/
    └── ...
...
同一周的简略记录（`.txt`）和详细周报（`report-*.md`）放在同一目录下，总结时读该周下所有 `.txt` 即可，`report-*.md` 作为归档参考。周报文件名带时间戳，多个 agent 同时生成也不会覆盖。
